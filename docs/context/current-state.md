...
- **Frontend Service (`/apps/frontend`)**: Scaffolded using Next.js App Router (TypeScript + Tailwind CSS v4). Setup dependencies, hybrid MSW interception router, HSL variables theme engine, global Layout with Outfit/JetBrains fonts, and custom glassmorphic components. Completed accessibility audits and optimized font sizes/color contrast values across light/dark themes, aligned onboarding structures, and disabled floating/bouncing transitions. All assets build cleanly under Turbopack. Added `suppressHydrationWarning` to both the `<html>` and `<body>` tags in [layout.tsx](file:///Users/parth/RAG/Document%20Intelligence%20Platform/apps/frontend/src/app/layout.tsx) to resolve React hydration mismatch warnings caused by the head script's synchronous theme loading and potential browser-extension attributes.
